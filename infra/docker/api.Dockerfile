...
#
# WHY THIS SHIPS SOURCE RATHER THAN A BUILD. `pnpm start` is `tsx src/index.ts`:
# the service's `build` script is `echo ok` because there is no compile step, so
# the runtime image needs the TypeScript and a `tsx` to read it. That is a
# deliberate trade -- no build output to keep in sync, at the cost of a heavier
# image and a slower boot than a compiled bundle.
#
# THE SLOWER BOOT IS THE PART THAT MATTERS OPERATIONALLY. A runtime-tsx service
# resolves its whole workspace on every cold start, and on a shared node that is
# the moment it is most CPU-starved. The startupProbe in infra/k8s/api.yaml is
# generous for exactly this reason; do not tighten it without measuring a real
# cold start on the cluster.
...
# --- dependencies ---------------------------------------------------------
# Manifests first, source second, so editing a handler rebuilds from here
# instead of reinstalling the whole workspace.
#
# Every package.json in api's dependency graph has to be listed. pnpm resolves
# `workspace:*` links at install time, so a missing manifest fails the install
# rather than producing a subtly broken tree -- which is the good failure mode.
FROM base AS deps
...
CMD ["node", "/repo/apps/api/node_modules/tsx/dist/cli.mjs", "src/index.ts"]
