...
#   docker build -f infra/docker/mcp.Dockerfile -t <registry>/jotacular-mcp:<tag> .
#
...
# routes -- `https://mcp.jotacular.com/mcp` -- and it must match character for
...
RUN pnpm install --frozen-lockfile --filter @jotacular/mcp...
...
COPY --from=deps --chown=jotacular:nodejs /repo/apps/mcp/node_modules ./apps/mcp/node_modules
COPY --chown=jotacular:nodejs apps/mcp ./apps/mcp
