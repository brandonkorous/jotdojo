...
jotDOJO does not own the cluster. It belongs to **sparx** — a different product
...
jotDOJO gets the `jotdojo` namespace and shares the cluster and the Postgres
*server*. Nothing else.
...
**`mcp.`**. The apex is the marketing site and it is served by the **same `jotdojo-web`
...
**The app is at `app.jotdojo.com` and the apex is not an option.** ADR-010 and
...
**2. The connection budget is shared and exhaustible.** jotDOJO has its own
`jotdojo` **database**, but it is on sparx's server, and that server's tier caps
...
`https://mcp.jotdojo.com/mcp` and must equal what `apps/mcp` advertises and what
...
In practice jotDOJO's *marginal* cost is far below that floor, because it is not
...
`DATABASE_URL` in every environment must point at the **restricted `jotdojo_app`
