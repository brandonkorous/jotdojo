...
export class SelectionEditor {
  ...
  resize(bigger: boolean) {
    if (!resizeSelection(this.sel, bigger)) return;
    this.ctx.index.invalidate(this.sel.selected);
    this.ctx.texts()?.refresh();
  ...
  tidyShape() {
    if (!tidySelection(this.sel)) return;
    this.ctx.index.invalidate(this.sel.selected);
    this.after();
  }
  ...
  private after() {
    ...
    this.ctx.overlay();
    this.ctx.onChange?.(this.sel.summary);
    this.ctx.onDelta({ remove: [], upsert: [...this.sel.selected], ...this.planes() });
  }
  ...
  finish() {
    if (this.sel.dragging) {
      if (this.sel.endDrag()) {
        ...
        // is an upsert by id -- of every kind at once.
        this.ctx.onDelta({ remove: [], upsert: [...this.sel.selected], ...this.planes() });
      }
      return;
    }
